...
# --- 설정 ---
st.set_page_config(page_title="Debug Factory", layout="wide")
st.title("🏭 디버깅 모드 공장 (속도 최적화)")
...
if video_path and st.button("🚀 분석 시작 (디버그 모드)"):
    st.divider()
    
    file_id = re.sub(r'\W+', '_', os.path.basename(video_path))
    ...
    print(f"=== 분석 시작: {total_duration}초 ===") # 터미널 로그

    for start in range(0, total_duration, 60):
        current_min = start // 60
        p_bar.progress(min(start / total_duration, 1.0))
        
        # UI 업데이트
        status_text.markdown(f"### ⚡ **{current_min}분대** 분석 중... (터미널을 확인하세요)")
        print(f"\n[진행] {current_min}분대 작업 시작")
        ...
        # 2. 프레임 추출
        frames = extract_smart_frames(video_path, save_path, start)
        ...
        while not success and retry_count < 5: # 무한 루프 방지 (최대 5회)
        ...
        if not success:
            print("   -> ❌ 최종 실패 (다음 구간으로 이동)")
            with open(save_file, "w", encoding="utf-8") as f: f.write("") # 실패해도 파일 만들어서 패스
